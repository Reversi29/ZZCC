...
def list_items(db: Session = Depends(get_db), limit=100, x_api_key: str = Header(None)):
    check(x_api_key)
    rows = db.query(Item).limit(limit).all()
    return R(data={"data": [md(r) for r in rows], "length": len(rows)})

@router.post("/Item", response_model=R)
def create_item(data: dict, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def get_item(name: str, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def update_item(name: str, data: dict, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
    m = db.query(Item).filter(Item.name == name).first()
    if not m: raise HTTPException(404, "Item not found")
    for k, v in data.items():
...
def delete_item(name: str, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
    m = db.query(Item).filter(Item.name == name).first()
...
def list_stock_entries(db: Session = Depends(get_db), limit=100, x_api_key: str = Header(None)):
    check(x_api_key)
    rows = db.query(StockEntry).limit(limit).all()
    return R(data={"data": [md(r) for r in rows], "length": len(rows)})

@router.post("/Stock Entry", response_model=R)
def create_stock_entry(data: dict, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def get_stock_entry(name: str, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def update_stock_entry(name: str, data: dict, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
    m = db.query(StockEntry).filter(StockEntry.name == name).first()
    if not m: raise HTTPException(404, "Stock Entry not found")
    for k, v in data.items():
...
def delete_stock_entry(name: str, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
    m = db.query(StockEntry).filter(StockEntry.name == name).first()
    if m: db.delete(m); db.commit()
...
def list_assets(db: Session = Depends(get_db), limit=100, x_api_key: str = Header(None)):
    check(x_api_key)
    rows = db.query(Asset).limit(limit).all()
    return R(data={"data": [md(r) for r in rows], "length": len(rows)})

@router.post("/Asset", response_model=R)
def create_asset(data: dict, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def get_asset(name: str, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def update_asset(name: str, data: dict, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def delete_asset(name: str, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
    m = db.query(Asset).filter(Asset.name == name).first()
    if m: db.delete(m); db.commit()
...
def stock_summary(db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
    rows, total = [], 0.0
...
def low_stock(db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
    warnings = []
    for it in db.query(Item).all():
