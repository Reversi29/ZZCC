...
def list_inspections(db: Session = Depends(get_db), limit=100, x_api_key: str = Header(None)):
    check(x_api_key)
...
def create_inspection(data: dict, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def get_inspection(name: str, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def update_inspection(name: str, data: dict, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def delete_inspection(name: str, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
    m = db.query(QualityInspection).filter(QualityInspection.name == name).first()
