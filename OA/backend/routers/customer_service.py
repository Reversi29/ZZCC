...
def list_tickets(db: Session = Depends(get_db), limit=100, x_api_key: str = Header(None)):
    check(x_api_key)
...
def create_ticket(data: dict, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def get_ticket(name: str, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def update_ticket(name: str, data: dict, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
...
def delete_ticket(name: str, db: Session = Depends(get_db), x_api_key: str = Header(None)):
    check(x_api_key)
    m = db.query(SupportTicket).filter(SupportTicket.name == name).first()
